...
so = SWOperator()
print('Compressor instantiate completed.')

fig, ax = plt.subplots()
x1,y1 = iter_test_random()
x2,y2 = iter_test_zipf()
ax.plot(x1, y1, color='green', label='random')
ax.plot(x2, y2, color='blue', label='zipf')
ax.set_xlabel('コマンド入力した回数', fontname ='MS Gothic')
ax.set_ylabel('1単語あたりの平均符号長', fontname ='MS Gothic')
ax.legend(loc=1)
plt.show()
